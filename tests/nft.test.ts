...
describe("fanout", async () => {
  ...
  describe("NFT membership model", () => {
    ...
    it("Distribute a Native Fanout with NFT Members", async () => {
      ...
      const memberDataAfterAgain1 = await connection.getAccountInfo(
        member1.wallet.publicKey
      );
      expect(memberDataAfterAgain1?.lamports).to.equal(firstSnapshot * 0.2);
      const membershipAccountAgain1 =
        await fanoutSdk.fetch<FanoutMembershipVoucher>(
          member1.voucher,
          FanoutMembershipVoucher
        );
      expect(membershipAccountAgain1.totalInflow.toString()).to.equal(
        `${firstSnapshot * 0.2}`
      );
      const sent2 = 10;
    ...
    it("Distributes a Fanout under a certain mint for NFT Members", async () => {
      ...
      const fanoutForMintAccountData = await fanoutSdk.fetch<FanoutMint>(
        fanoutForMint,
        FanoutMint
      );
      const distBot = new Keypair();
      ...
      await mint.mintTo(
        fanoutForMintAccountData.tokenAccount,
        authorityWallet,
        [],
        sent
      );
      ...
      let [fanoutForMintMembershipVoucher, _] =
      ...
      let fanoutForMintAccountDataAfter = await fanoutSdk.fetch<FanoutMint>(
        fanoutForMint,
        FanoutMint
      );
      let fanoutForMintMember1VoucherAfter =
        await fanoutSdk.fetch<FanoutMembershipMintVoucher>(
          fanoutForMintMembershipVoucher,
          FanoutMembershipMintVoucher
        );
      expect(
        (await connection.getTokenAccountBalance(fanoutMintMember1TokenAccount))
          .value.amount
      ).to.equal(`${sent * 0.2}`);
      expect(
        (await connection.getTokenAccountBalance(fanoutMintMember2TokenAccount))
          .value.amount
      ).to.equal(`${sent * 0.2}`);
      expect(fanoutForMintAccountDataAfter.totalInflow.toString()).to.equal(
        `${sent}`
      );
      expect(
        fanoutForMintAccountDataAfter.lastSnapshotAmount.toString()
      ).to.equal(`${sent - sent * 0.2 * 2}`);
      expect(fanoutForMintMember1VoucherAfter.lastInflow.toString()).to.equal(
        `${sent}`
      );
      ...
      expect(
        (await connection.getTokenAccountBalance(fanoutMintMember3TokenAccount))
          .value.amount
      ).to.equal(`${sent * 0.2 + sent2 * 0.2}`);
    });
  });
